          ...
          {!checking && status?.installed && (
            <>
              <div className="flex flex-col gap-2">
                ...
                {/* Current configured */}
                {(status?.config?.provider?.Mayday?.options?.baseURL || status?.config?.provider?.["mayday"]?.options?.baseURL) && (
              ...
              <div className="grid grid-cols-1 gap-2 sm:flex sm:items-center">
                ...
                <Button variant="outline" size="sm" onClick={handleReset} disabled={!status.hasMayday} loading={restoring}>
...
export default function OpenCodeToolCard({ tool, isExpanded, onToggle, baseUrl, apiKeys, activeProviders, cloudEnabled, initialStatus, tunnelEnabled, tunnelPublicUrl, tailscaleEnabled, tailscaleUrl }) {
  ...
  const subagentModel = subagentModelOverride ?? (() => {
    ...
    if (model.startsWith("Mayday/")) return model.replace("Mayday/", "");
    if (model.startsWith("mayday/")) return model.replace("mayday/", "");
    return "";
  })();
  ...
  const saveModels = async (models) => {
    try {
      const keyToUse = (selectedApiKey && selectedApiKey.trim())
        ? selectedApiKey
        : (!cloudEnabled ? "sk_Mayday" : selectedApiKey);
  ...
  const getConfigStatus = () => {
    ...
    if (!status.hasMayday) return "not_configured";
    const url = status.config?.provider?.Mayday?.options?.baseURL
      || status.config?.provider?.["mayday"]?.options?.baseURL
      || "";
  ...
  const getManualConfigs = () => {
    ...
    return [{
      ...
      content: JSON.stringify({
        ...
        model: `Mayday/${activeModelToShow}`,
        agent: {
          explorer: {

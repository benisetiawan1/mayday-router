              ...
              <div className="flex flex-col gap-3 p-4 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-text-muted">Manual configuration is still available if Mayday is deployed on a remote server.</p>
                  </div>
                </div>
            ...
            <>
              <div className="flex flex-col gap-2">
                ...
                <div className="grid grid-cols-1 gap-1.5 sm:grid-cols-[8rem_auto_1fr] sm:items-center sm:gap-2">
                  ...
                  <BaseUrlSelect currentUrl={openclawStatus?.settings?.models?.providers?.["Mayday"]?.baseUrl || ""} value={customBaseUrl || getDisplayUrl()}
                ...
                {/* Current configured */}
                {openclawStatus?.settings?.models?.providers?.["Mayday"]?.baseUrl && (
              ...
              <div className="grid grid-cols-1 gap-2 sm:flex sm:items-center">
                ...
                <Button variant="outline" size="sm" onClick={handleResetSettings} disabled={!openclawStatus?.hasMayday} loading={restoring}>
...
export default function OpenClawToolCard({
  ...
  const getConfigStatus = () => {
    ...
    const currentProvider = openclawStatus.settings?.models?.providers?.["Mayday"];
  ...
  if (openclawStatus?.installed && !hasInitializedModel) {
    ...
    const provider = openclawStatus.settings?.models?.providers?.["Mayday"];
    if (provider) {
      const primaryModel = openclawStatus.settings?.agents?.defaults?.model?.primary;
      if (primaryModel && !selectedModel) setSelectedModel(primaryModel.replace("Mayday/", ""));
  ...
  const getManualConfigs = () => {
    ...
    const settingsContent = {
      agents: {
        defaults: {
          model: {
            primary: `Mayday/${selectedModel || "provider/model-id"}`,
          },
        },
      },
      models: {

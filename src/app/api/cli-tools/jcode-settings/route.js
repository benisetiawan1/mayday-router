...
const getProviderEnvPath = () => {
  ...
  return path.join(configDir, "jcode", "provider-Mayday.env");
};
...
const hasMaydayConfig = (config) => {
  ...
  if (providers["Mayday"]) return true;
...
export async function GET() {
  ...
  const hasMayday = hasMaydayConfig(config);
...
export async function POST(request) {
  try {
    ...
    env.JCODE_Mayday_API_KEY = apiKey;
    await writeProviderEnv(env);

    return NextResponse.json({
      success: true,
      message: "jcode configured successfully. Use: jcode --provider-profile Mayday",
      configPath: getConfigPath(),
    });
  } catch (error) {
...
export async function DELETE() {
  try {
    ...
    delete config.providers["Mayday"];

    await writeConfig(config);

    const env = await readProviderEnv();
    delete env.JCODE_Mayday_API_KEY;
    await writeProviderEnv(env);

    return NextResponse.json({
      success: true,
      message: "Mayday configuration removed from jcode",
    });
  } catch (error) {

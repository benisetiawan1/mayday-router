...
// Check if settings has Mayday customModels
const hasMaydayConfig = (settings) => {
  if (!settings || !settings.customModels) return false;
  return settings.customModels.some(m => m.id?.startsWith("custom:Mayday"));
};
...
// POST - Update Mayday customModels (merge with existing settings)
...
export async function POST(request) {
  try {
    ...
    // Remove all existing Mayday configs
    settings.customModels = settings.customModels.filter(m => !m.id?.startsWith("custom:Mayday"));
...
// DELETE - Remove Mayday customModels only (keep other settings)

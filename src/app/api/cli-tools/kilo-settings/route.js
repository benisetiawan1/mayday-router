...
const hasMaydayConfig = (auth) => {
  if (!auth) return false;
  const entry = auth["openai-compatible"] || auth["mayday"];
  if (!entry) return false;
  const baseUrl = entry.baseUrl || entry.baseURL || "";
  return baseUrl.includes("localhost") || baseUrl.includes("127.0.0.1") || baseUrl.includes("mayday");
};
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      hasMayday: hasMaydayConfig(auth),
...
export async function POST(request) {
  try {
    ...
    try {
      ...
      vscode["kilocode.customProvider"] = { name: "Mayday", baseURL: normalizedBaseUrl, apiKey };
...
export async function DELETE() {
  try {
    ...
    return NextResponse.json({ success: true, message: "Mayday settings removed from Kilo Code" });
  } catch (error) {

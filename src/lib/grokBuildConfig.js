...
const previousDefaultRegExp = /^# mayday-prev-default = "([^"]*)"[ \t]*\r?\n?/m;
const previousSubagentRegExp = (type) =>
  new RegExp(
    `^# mayday-prev-subagent-${escapeRegExp(type)} = "([^"]*)"[ \\t]*\\r?\\n?`,
    "m",
  );
...
function buildModelSection({ slot, model, baseUrl, apiKey, contextWindow, name }) {
  const lines = [
    ...
    `description = ${tomlString("Routed via Mayday gateway")}`,
...
function rememberPreviousDefault(toml) {
  ...
  return insertMarker(toml, `# mayday-prev-default = ${tomlString(current)}\n`);
}
...
function rememberPreviousSubagent(toml, type) {
  ...
  return insertMarker(
    toml,
    `# mayday-prev-subagent-${type} = ${tomlString(previous)}\n`,
  );
}

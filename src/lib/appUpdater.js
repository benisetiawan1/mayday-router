  ...
  try {
    const mitmPidFile = path.join(
      process.platform === "win32"
        ? path.join(process.env.APPDATA || "", "mayday")
...
// Collect PIDs of all Mayday-related processes (excluding current)
  ...
  if (platform === "win32") {
    try {
      ...
      lines.forEach(line => {
        ...
        // Match anything running from Mayday install dir or wrapper cli.js
        const isAppProcess = lower.includes("mayday") ||
    ...
    try {
      ...
      output.split("\n").forEach(line => {
        const isAppProcess = line.includes("mayday") ||
...
// Resolve npx/Mayday binary to relaunch after update (cross-platform)
function resolveRelaunchCommand() {
  const isWin = process.platform === "win32";
  // Prefer `npx Mayday` — works regardless of global bin path changes after npm i -g

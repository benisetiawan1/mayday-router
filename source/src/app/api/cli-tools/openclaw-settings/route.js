...
// Check if settings has Mayday config
const hasMaydayConfig = (settings) => {
  if (!settings || !settings.models || !settings.models.providers) return false;
  return !!settings.models.providers["mayday"];
};

// Read per-agent models.json and return current model id (without "mayday/" prefix)
const readAgentModel = async (agentDir) => {
  try {
    const modelsPath = path.join(agentDir, "models.json");
    const content = await fs.readFile(modelsPath, "utf-8");
    const data = JSON.parse(content);
    const models = data?.providers?.["mayday"]?.models;
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      hasMayday: hasMaydayConfig(settings),
...
// POST - Update Mayday settings (merge with existing settings)
export async function POST(request) {
  try {
    ...
    const fullModelId = `mayday/${model}`;

    // Remove all old mayday/* entries from agents.defaults.models
    for (const k of Object.keys(settings.agents.defaults.models)) {
      if (k.startsWith("mayday/")) delete settings.agents.defaults.models[k];
    }
    ...
    // Add fresh mayday models to allowlist
    allModelIds.forEach((m) => {
      settings.agents.defaults.models[`mayday/${m}`] = {};
    });

    // Remove old mayday model from each agent in agents.list. The
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        if (resolveAgentModel(agent.model).startsWith("mayday/")) {
    ...
    // Update models.providers.mayday with all models
    settings.models.providers["mayday"] = {
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        ...
        if (agentModel) return { ...agent, model: `mayday/${agentModel}` };
        return agent;
      });
...
export async function DELETE() {
  try {
    ...
    // Remove Mayday from models.providers
    if (settings.models && settings.models.providers) {
      delete settings.models.providers["mayday"];
    ...
    // Remove mayday models from agents.defaults.models allowlist
    if (settings.agents?.defaults?.models) {
      const keysToRemove = Object.keys(settings.agents.defaults.models).filter((k) => k.startsWith("mayday/"));
    ...
    // Reset agents.defaults.model.primary if it uses mayday
    if (settings.agents?.defaults?.model?.primary?.startsWith("mayday/")) {

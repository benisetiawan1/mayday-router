              ...
              <div className="flex flex-col gap-3 p-4 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-text-muted mt-2">Manual configuration is still available if Mayday is deployed on a remote server.</p>
                  </div>
                </div>
          ...
          {!checkingJcode && jcodeStatus?.installed && (
            <>
              <div className="flex flex-col gap-2">
                ...
                {/* Current configured */}
                {jcodeStatus?.config?.providers?.["Mayday"]?.base_url && (
                ...
                <div className="flex flex-col gap-1 p-3 bg-blue-500/5 border border-blue-500/20 rounded-lg">
                  ...
                  <code className="text-xs font-mono text-text-muted">jcode --provider-profile Mayday</code>
                  <code className="text-xs font-mono text-text-muted">jcode --provider-profile Mayday --model {selectedModel || "cc/claude-opus-4-7"}</code>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 gap-2 sm:flex sm:items-center">
                ...
                <Button variant="outline" size="sm" onClick={handleResetSettings} disabled={!jcodeStatus?.hasMayday} loading={restoring}>
...
export default function JcodeToolCard({
  ...
  const selectedModel = selectedModelOverride ?? jcodeStatus?.config?.providers?.["Mayday"]?.default_model ?? "";
  ...
  const getConfigStatus = () => {
    ...
    if (!jcodeStatus?.hasMayday) return "not_configured";
    const currentProvider = jcodeStatus.config?.providers?.["Mayday"];
  ...
  const getManualConfigs = () => {
    const keyToUse = (selectedApiKey && selectedApiKey.trim())
      ? selectedApiKey
      : (!cloudEnabled ? "sk_Mayday" : "<API_KEY_FROM_DASHBOARD>");

    const configToml = `[providers.Mayday]
type = "openai-compatible"
base_url = "${getEffectiveBaseUrl()}"
auth = "bearer"
api_key_env = "JCODE_Mayday_API_KEY"
env_file = "provider-Mayday.env"
default_model = "${selectedModel || "cc/claude-opus-4-7"}"
requires_api_key = true

[[providers.Mayday.models]]
id = "${selectedModel || "cc/claude-opus-4-7"}"`;

    const envContent = `JCODE_Mayday_API_KEY="${keyToUse}"`;
    ...
    return [
      ...
      {
        filename: "~/.config/jcode/provider-Mayday.env",
        content: envContent,
      },
    ];
  };

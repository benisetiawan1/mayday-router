 ...
 * @param {string} packageName  npm package name (e.g. "mayday")
 * @returns {string|null}
 */
export function updateAndRestartCommand(runtime, packageName) {
  ...
  switch (runtime) {
    case "pm2":
      ...
      const pm2Name = readPm2ProcessName() || "mayday";
    ...
    case "screen":
      // Re-launch in a detached screen window named "mayday".
      // The user's existing screen session keeps running — they can
      // `screen -r mayday` to attach to the new instance.
      return `${installCmd} && screen -dmS mayday mayday`;
    case "tmux":
      // Re-launch in a detached tmux session named "mayday".
      // User can `tmux attach -t mayday` to attach to the new instance.
      return `${installCmd} && tmux new-session -d -s mayday 'mayday'`;
    case "docker":
...
function readSystemdUnitName() {
  ...
  try {
    const exe = process.execPath.split("/").pop() || "mayday";
    return `${exe}.service`;
  } catch {
    return null;
  }
